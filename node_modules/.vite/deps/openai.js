		...
		const runToolCall = async (toolCall) => {
			...
			if (!fn) return {
				message: {
					role,
					tool_call_id,
					content: `Invalid tool_call: ${JSON.stringify(name)}. Available options are: ${Object.keys(functionsByName).map((name) => JSON.stringify(name)).join(", ")}. Please try again`
				},
				functionCalled: false
			};
			if (singleFunctionToCall && singleFunctionToCall !== name) return {
				message: {
					role,
					tool_call_id,
					content: `Invalid tool_call: ${JSON.stringify(name)}. ${JSON.stringify(singleFunctionToCall)} requested. Please try again`
				},
				functionCalled: false
			};
			let rawContent;
			...
			return {
				message: {
					role,
					tool_call_id,
					content: __classPrivateFieldGet(this, _AbstractChatCompletionRunner_instances, "m", _AbstractChatCompletionRunner_stringifyFunctionCallResult).call(this, rawContent)
				},
				functionCalled: true
			};
		};
...
var AzureOpenAI = class extends OpenAI {
	...
	*/
	...
	async authHeaders(opts, schemes) {
		...
		if (security.bearerAuth && typeof this._options.apiKey === "string") return buildHeaders([{ "api-key": this.apiKey }]);
...
function deriveBedrockBaseURL(awsRegion) {
	...
	if (!region) throw new OpenAIError("Must provide one of the `baseURL` or `awsRegion` arguments, or set the `AWS_BEDROCK_BASE_URL`, `AWS_REGION`, or `AWS_DEFAULT_REGION` environment variable.");
...
function normalizeBedrockBaseURL(baseURL) {
	...
	if (responsesMatch?.index !== void 0) url.pathname = url.pathname.slice(0, responsesMatch.index) || "/";
...
function addBedrockOutputText(response) {
	if (!Object.getOwnPropertyDescriptor(response, "output_text")) addOutputText(response);
	return response;
}
...
var BedrockOpenAI = class extends OpenAI {
	...
	*/
	constructor({ baseURL = readEnv("AWS_BEDROCK_BASE_URL"), apiKey, awsRegion = readEnv("AWS_REGION") ?? readEnv("AWS_DEFAULT_REGION"), bedrockTokenProvider, adminAPIKey, workloadIdentity, ...opts } = {}) {
		if (adminAPIKey || workloadIdentity) throw new OpenAIError("BedrockOpenAI only supports Bedrock bearer token authentication.");
		if (apiKey === void 0 && !bedrockTokenProvider) apiKey = readEnv("AWS_BEARER_TOKEN_BEDROCK") ?? null;
		if (typeof apiKey === "function") throw new OpenAIError("Pass refreshable Bedrock credentials via `bedrockTokenProvider`, not `apiKey`.");
		if (apiKey && bedrockTokenProvider) throw new OpenAIError("The `apiKey` and `bedrockTokenProvider` arguments are mutually exclusive; only one can be passed at a time.");
		if (!apiKey && !bedrockTokenProvider) throw new OpenAIError("Missing credentials. Please pass an `apiKey` or `bedrockTokenProvider`, or set the `AWS_BEARER_TOKEN_BEDROCK` environment variable.");
	...
	async prepareOptions(options) {
		const security = options.__security ?? { bearerAuth: true };
		if (security.adminAPIKeyAuth && !security.bearerAuth) await this._callApiKey();
	...
	async authHeaders(opts, schemes) {
		...
		if ((security.bearerAuth || security.adminAPIKeyAuth) && this.apiKey !== null) return buildHeaders([{ Authorization: `Bearer ${this.apiKey}` }]);
		return super.authHeaders(opts, security);
	}
	withOptions(options) {
...
//#endregion
export { APIConnectionError, APIConnectionTimeoutError, APIError, APIPromise, APIUserAbortError, AuthenticationError, AzureOpenAI, BadRequestError, BedrockOpenAI, ConflictError, InternalServerError, InvalidWebhookSignatureError, NotFoundError, OAuthError, OpenAI, OpenAI as default, OpenAIError, PagePromise, PermissionDeniedError, RateLimitError, SubjectTokenProviderError, UnprocessableEntityError, toFile };
